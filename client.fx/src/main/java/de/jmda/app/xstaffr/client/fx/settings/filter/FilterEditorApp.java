...
public class FilterEditorApp extends FXApp
{
	private final static Logger LOGGER = LogManager.getLogger(FilterEditorApp.class);

	private FilterEditor fxView;

	@Override protected FXView getFXView()
	{
		if (fxView == null)
		{
			fxView = CDI.current().select(FilterEditor.class).get();
