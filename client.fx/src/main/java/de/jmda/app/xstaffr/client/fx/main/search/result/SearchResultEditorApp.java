...
public class SearchResultEditorApp extends FXApp
{
	private final static Logger LOGGER = LogManager.getLogger(SearchResultEditorApp.class);

	private SearchResultEditor fxView;

	@Override protected FXView getFXView()
	{
		if (fxView == null)
		{
			fxView = CDI.current().select(SearchResultEditor.class).get();
